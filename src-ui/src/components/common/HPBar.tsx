...
export function HPBar({ percent, size = "sm", thin = false }: HPBarProps) {
    ...
    return (
      <div className="flex items-center gap-2">
        <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-raised">
          <div
            data-fill
            className={`h-full rounded-full transition-all duration-300 ${hpColor(percent)}`}
            style={{ width: `${clamped}%` }}
          />
        </div>
  ...
  return (
    <div className={`relative w-full ${h} overflow-hidden rounded-full bg-raised`}>
      <div
        data-fill
        className={`absolute top-0 left-0 ${h} rounded-full transition-all duration-300 ${hpColor(percent)}`}
        style={{ width: `${clamped}%` }}
      />
